...
s2_sa = pd.read_csv("results/strategy2_stress_a.csv")
all_ok &= check("unknown_detection_auroc in S2 stress",
                "unknown_detection_auroc" in s2_sa.columns)
if "unknown_detection_auroc" in s2_sa.columns:
    all_ok &= check("S2 unknown_detection_auroc has 2 groups",
                    len(s2_sa["unknown_detection_auroc"].dropna()) == 2)
...
print("\n6. Stress B: feature-shift statistics reproducible?")
...
all_ok &= check("stress_b_feature_shift.csv exists", os.path.exists(shift_path))
if os.path.exists(shift_path):
    shift = pd.read_csv(shift_path)
    required_shift_cols = [
        "feature", "mean_unsw", "variance_unsw", "mean_cicids",
        "variance_cicids", "standardized_mean_diff",
    ]
    all_ok &= check("feature-shift table has 49 features", len(shift) == 49)
    all_ok &= check("feature-shift table has required statistics",
                    all(c in shift.columns for c in required_shift_cols))
