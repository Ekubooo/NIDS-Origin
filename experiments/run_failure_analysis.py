...
def analyze_stress_b_feature_shift():
    """Extract reproducible Stress B feature-shift statistics."""
    shift_path = "results/stress_b_feature_shift.csv"
...
def analyze_strategy2_stress_a():
    """Extract Strategy 2 open-set disagreement results."""
    path = "results/strategy2_stress_a.csv"
    if os.path.exists(path):
        return pd.read_csv(path)
    return None


def write_failure_analysis(per_class, tau_data, stress_b, feature_shift,
                           smote_data, s2_stress_a):
    ...
    if feature_shift is not None and len(feature_shift) > 0:
        top_shift = feature_shift.head(5)
        lines.append("- **Feature-level explanation**: The largest reproducible shifts are:\n")
        lines.append("\n  | Feature | UNSW mean | CICIDS mean | Std. mean diff |\n")
        lines.append("  |---------|-----------|-------------|----------------|\n")
        for _, row in top_shift.iterrows():
            lines.append(
                f"  | {row['feature']} | {row['mean_unsw']:.3f} | "
                f"{row['mean_cicids']:.3f} | {row['standardized_mean_diff']:.3f} |\n"
            )
        lines.append("\n")
    else:
        lines.append("- **Feature-level explanation**: Run `python experiments/analyze_stress_b_shift.py`\n")
        lines.append("  to generate `results/stress_b_feature_shift.csv` for reproducible source-target statistics.\n")
    ...
    if s2_stress_a is not None and "unknown_detection_auroc" in s2_stress_a.columns:
        unk_mean = s2_stress_a["unknown_mean_disagreement"].mean()
        known_mean = s2_stress_a["known_mean_disagreement"].mean()
        unk_auroc = s2_stress_a["unknown_detection_auroc"].mean()
        lines.append(f"- **Unknown class disagreement**: mean {unk_mean:.4f} vs "
                     f"known mean {known_mean:.4f}.\n")
        lines.append(f"- **Unknown detection AUROC**: {unk_auroc:.4f} on Stress A, "
                     "showing that disagreement also separates unknown from known inputs.\n")
...
def main():
    ...
    feature_shift = analyze_stress_b_feature_shift()
    smote_data = analyze_smote()
    s2_stress_a = analyze_strategy2_stress_a()

    write_failure_analysis(per_class, tau_data, stress_b, feature_shift,
                           smote_data, s2_stress_a)
