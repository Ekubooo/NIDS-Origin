...
def analyze_missing_values(df):
    """Analyze missing values after treating inf as missing."""
    missing = df.isnull().sum()
    numeric_df = df.select_dtypes(include=[np.number])
    posinf = pd.Series(0, index=df.columns, dtype="int64")
    neginf = pd.Series(0, index=df.columns, dtype="int64")
    if not numeric_df.empty:
        posinf.loc[numeric_df.columns] = np.isposinf(numeric_df).sum()
        neginf.loc[numeric_df.columns] = np.isneginf(numeric_df).sum()
    inf_count = posinf + neginf
    total_missing = missing + inf_count
    missing_pct = (total_missing / len(df) * 100).round(2)
    report = pd.DataFrame({
        "count": total_missing,
        "pct": missing_pct,
        "raw_nan_count": missing,
        "inf_count": inf_count,
    })
    report = report[report["count"] > 0].sort_values("count", ascending=False)

    print(f"\n=== Missing / Inf Values ({len(report)} columns affected) ===")
    print("Note: preprocessing replaces +/-inf with NaN before imputation.")
...
def analyze_feature_distribution_by_class(df):
    ...
    for i, feat in enumerate(available):
        ...
        sns.boxplot(data=plot_df, x="class", y="value", hue="class", ax=axes[i],
                    palette="Set2", fliersize=1, linewidth=0.5, legend=False)
...
def save_notes(df, missing_report, decisions, l7_nonint, class_dist,
    ...
    lines.append("The preprocessing pipeline replaces `+/-inf` with `NaN` before imputation, "
                 "so the table reports both raw nulls and infinite values.\n")
    if len(missing_report) == 0:
        lines.append("No missing or infinite values found.\n")
    else:
        lines.append("| Column | Total Missing After Inf Conversion | Raw NaN | Inf | Missing % | Decision |\n")
        lines.append("|--------|------------------------------------|---------|-----|-----------|----------|\n")
        for col in missing_report.index:
            cnt = missing_report.loc[col, "count"]
            pct = missing_report.loc[col, "pct"]
            raw_nan = missing_report.loc[col, "raw_nan_count"]
            inf_count = missing_report.loc[col, "inf_count"]
            dec = decisions.get(col, "Fill with median")
            lines.append(f"| {col} | {int(cnt):,} | {int(raw_nan):,} | "
                         f"{int(inf_count):,} | {pct:.2f}% | {dec} |\n")
