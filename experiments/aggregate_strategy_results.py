...
def _baseline_clean_f1():
    """Read the selected RandomForest clean Macro-F1 from baseline results."""
    baseline = _load("results/baseline_results.csv")
    if baseline is None or "model" not in baseline.columns or "macro_f1" not in baseline.columns:
        return np.nan
    rf = baseline[baseline["model"] == "RandomForest"]
    if len(rf) == 0:
        return np.nan
    return float(rf["macro_f1"].iloc[0])
...
def main():
    ...
    clean_f1 = _baseline_clean_f1()

    rows = [{
        "Method": "Baseline (no strategy)",
        "Clean Full F1": round(clean_f1, 4) if not np.isnan(clean_f1) else np.nan,
        "Coverage": 1.0,
        "Accepted F1": np.nan,
    ...
    coverage_numeric = df_report["Coverage"].astype(object).copy()
    coverage_numeric.loc[coverage_numeric == "1.000 (no rejection)"] = 1.0
    coverage_numeric = pd.to_numeric(coverage_numeric, errors="coerce")
    df_report["Coverage"] = coverage_numeric.apply(
